...
public class AddPORPage extends BasePage {
    ...
    public By ProjectIDValueIcon = By.xpath(
            "//input[@sname='PJ:Project ID']//parent::*//parent::*//child::input[@title='Click for detailed info']"
    );
    ...
    public By porRequestQueue = By.xpath(
            "//label[text()='POR:POR Request Queue']"
    );
    ...
    public By ProjectSearchTextboxSearchButton = By.xpath(
            "//input[@id='btnSearch0']"
    );
    ...
    public PORTrackerPage verifyMandatoryMessageAlert(String ringCode)
            throws Exception {
    ...
    public AddPORPage verifyAddPORFunctionality(String ringCode)
            throws Exception {
        addNewPORTracker(new Por(ringCode));
        return new AddPORPage(driver);
    }
    ...
    public Por addNewPORTracker(Por por) throws Exception {
        ...
        selectDropdownOption(
                selectionBoxBySname("POR:Requested Plan Status - New Request").get(0),
                por.planStatus
        );
        selectDropdownOption(
                selectionBoxBySname("POR:Objective").get(0),
                por.objective
        );
    ...
    public String changePORRequestQueue() throws Exception {
        ...
        String ProjectIDValueText = inputBoxDataBySname("PJ:Project ID")
                .getAttribute("origval");
    ...
    public String changePORRequestQueue1(Por por) throws Exception {
        ...
        scrollToElement(find(porRequestQueue));
        selectDropdownOption(
                selectionBoxBySname("POR:POR Request Queue").get(0),
                por.queueStatus
        );
        ...
        if (ProjectIDValueText == null || ProjectIDValueText.isEmpty()) return "";
        return ProjectIDValueText;
    }
    ...
    public String changePORRequestQueueforBundle() throws Exception {
        ...
        // sleep(30);
        dropDownValueSelection("POR:Bundling Disposition", "Unbundled");
        WebElement lock1 = driver.findElement(
                By.xpath("//div[@id='idx93_imglock']")
        );
        String locktext1 = lock1.getAttribute("class");
        System.out.println(locktext1);
        if (locktext1.equalsIgnoreCase("lock-icon")) {
    ...
    public String getProjectCompletionObjectiveValuePOR() throws Exception {
        String ObjValue = textAreaBySname("POR:Completion Objective (Short)")
                .getAttribute("value");
        return ObjValue;
    }

    public String cbeForecastDetails(String cbe) throws Exception {
        WebElement cbeElement = selectionBoxBySname(cbe).get(0);
        scrollToElement(cbeElement);
        String one = getFirstSelectedOptionInDropdown(cbeElement);
    ...
    public void setFiberOption(String option) throws Exception {
        waitForPageToLoad();
        WebElement fiber = selectionBoxBySname("POR:Fiber Feasibility").get(0);
        scrollToElement(fiber);
        selectDropdownOption(fiber, option);
        click(find(applyButton));
        waitForPageToLoad();
    }

    public void setDarkFiberOption(String option) throws Exception {
        waitForPageToLoad();
        WebElement fiber = selectionBoxBySname("POR:Dark Fiber Feasibility").get(0);
        scrollToElement(fiber);
        selectDropdownOption(fiber, option);
        click(find(applyButton));
        waitForPageToLoad();
    ...
    public String getFiberDate() throws Exception {
        waitForPageToLoad();
        String modifiedDatePath = inputBoxXpathBySname("POR:Fiber Release Date");
        scrollToElement(selectionBoxBySname("POR:Fiber Feasibility").get(0));
        String updatedDate = getDocumentTextByXpathJs(modifiedDatePath);
        System.out.println("_" + updatedDate + "_");
        return updatedDate;
    }

    public String getDarkFiberDate() throws Exception {
        waitForPageToLoad();
        String modifiedDatePath = inputBoxXpathBySname(
                "POR:Dark Fiber Release Date"
        );
        scrollToElement(selectionBoxBySname("POR:Fiber Feasibility").get(0));
        String updatedDate = getDocumentTextByXpathJs(modifiedDatePath);
        System.out.println("_" + updatedDate + "_");
    ...
    public boolean setNotes(String field, String note) throws Exception {
        scrollToElement(selectionBoxBySname("POR:Fiber Feasibility").get(0));
        WebElement textArea = textAreaBySname(field);
        setText(textArea, note);
        click(find(applyButton));
        sleep(5);
        textArea = textAreaBySname(field);
        String text = getText(textArea);
        System.out.println("_" + text + "_");
        if (text.equals(note)) return true; else return false;
    }
}
