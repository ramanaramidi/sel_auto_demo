...
public class ProjectHelper {
    ...
    public String deleteProjectTracker(String projectId){
        ...
        ApiResponse response = projectService.getProjectTrackerIdsForSite(projectCode[0]);
        if(response.responseCode == 200) {
            List<Integer> trackerKeys = JsonPath.read(response.responseBody, "[?(@.TRACKOR_KEY== '" + projectId + "')].TRACKOR_ID");
    ...
    public Project getActiveProject(boolean isForceSeeding,Ring ring, Site site, Por por){
        if(isForceSeeding){
            return createActiveProjectFromScratch(ring,site,por);
        }
        else {
            Trackers trackerData = DataCollector.getTrackerData(SeedingDataTypeEnum.ActiveProject);
            if(trackerData!=null){
                String projectId = trackerData.getTrackerId();
                String[] siteCode = projectId.split("-");
                System.out.println("Site Code:: "+siteCode[0]);
                ApiResponse response = siteHelper.getSiteTrackerDetailsBySiteIdAndCustomFields(siteCode[0],"RingTracker.xitor_key");
                if(response!=null){
                    String ringId = JsonPath.read(response.responseBody, "[0].['RingTracker.TRACKOR_KEY']");
    ...
    public Project getActiveProject(boolean isForceSeeding,Ring ring, Site site){
        if(isForceSeeding){
    ...
    public Project createActiveProjectFromScratch(Ring ring, Site site){
        ...
        ApiResponse response = projectService.getProjectTrackerIdsForSite(site.siteId);
        if(response.responseCode == 200) {
            String trackerKey = JsonPath.read(response.responseBody, "[0].TRACKOR_KEY");
            Project project = new Project(trackerKey,porData.porId,site.siteId,ring.ringId,porData);
    ...
    public Project createActiveProjectFromScratch(Ring ring, Site site, Por porData){
        ProjectService projectService = new ProjectService();
        site = siteHelper.createActiveRingAndPrimaryActiveSite(ring,site);
        //Por porData = new Por(site.ringId);
        porData = porHelper.createAPor(porData);
    ...
    public Project getNewProject(boolean isForceSeeding,Ring ring, Site site){
        if(isForceSeeding){
            ...
            Trackers trackerData = DataCollector.getTrackerData(SeedingDataTypeEnum.NewProject);
            if(trackerData!=null){
                String projectId = trackerData.getTrackerId();
                String[] siteCode = projectId.split("-");
                System.out.println("Site Code:: "+siteCode[0]);
    ...
    public boolean uploadDocument(String trackerId,String field, String path,String fileName){
        ProjectService projectService = new ProjectService();
        ApiResponse response = projectService.uploadNTPFile(trackerId,field,path,fileName);
        if(response.responseCode == 200)
            return true;
        else
            return false;
